...
def compute_metrics(df: pd.DataFrame, prices: Dict[str, float]):
    rows = []
...
def api_data():
    ...
    rows = compute_metrics(df, prices)
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="timestamp">Actualizado: <span id="ts">—</span></p>
    ...
    <script>
        ...
        async function load() {
            ...
            try {
                ...
                document.getElementById('ts').textContent = fmtTime(data.timestamp);
                renderSummary(data.summary);
                renderGrid(data.rows);
            } catch(e) { console.error(e); }
            finally {
                if(btn) btn.classList.remove('loading');
                if(txt) txt.textContent = 'Actualizar Precios';
            }
        }
        ...
        function renderSummary(s) {
            ...
            document.getElementById('summary').innerHTML = `
                <div class="summary-card ${pnlClass}">
                    <h3>P&L TOTAL</h3>
                    <div class="value">${fmtMoney(s.total_pnl)}</div>
                    <p class="subtext">(${fmt(s.total_pnl_pct)}%)</p>
                </div>
                <div class="summary-card">
                    <h3>EXPOSICIÓN</h3>
                    <div class="value">${fmtMoney(s.exposure_total)}</div>
                    <p class="subtext">de $800 cap</p>
                </div>
                <div class="summary-card">
                    <h3>TRADES ACTIVOS</h3>
                    <div class="value">${s.active}</div>
                    <p class="subtext">de ${s.active} en plan</p>
                </div>
                <div class="summary-card">
                    <h3>PROB. WIN PROMEDIO</h3>
                    <div class="value">${fmt(s.avg_prob)}%</div>
        ...
        function renderGrid(rows) {
            ...
            rows.forEach(r => {
                ...
                const badgeClass = r.pnl >= 0 ? 'badge-pos' : 'badge-neg';
                ...
                card.innerHTML = `
                    ...
                    <div class="trade-body">
                        <div class="pnl-banner ${pnlClass}">${fmtMoney(r.pnl)} (${fmt(r.pnl_pct)}%)</div>
                        <div class="price-row"><span class="price-label">Entry</span><span class="price-value entry-price">${fmtMoney(r.entry)}</span></div>
                        <div class="price-row"><span class="price-label">Current Price</span><span class="price-value current-price">${fmtMoney(r.current)}</span></div>
                        <div class="price-row"><span class="price-label">Target (TP)</span><span class="price-value tp-price">${fmtMoney(r.tp)}</span></div>
                        <div class="price-row"><span class="price-label">Stop (SL)</span><span class="price-value sl-price">${fmtMoney(r.sl)}</span></div>
                        <div class="progress-container">
                            <div class="progress-label"><span>Progreso SL → TP</span><span>${fmt(r.progress_pct)}%</span></div>
                            <div class="progress-bar"><div class="progress-fill" style="width:${fmt(r.progress_pct)}%"></div></div>
                        </div>
                        <div class="mini-grid">
                            <div class="mini"><div class="mini-label">Dist a TP</div><div class="mini-value">${fmtMoney(r.dist_tp)}</div></div>
                            <div class="mini"><div class="mini-label">Dist a SL</div><div class="mini-value">${fmtMoney(r.dist_sl)}</div></div>
                            <div class="mini"><div class="mini-label">Qty</div><div class="mini-value">${r.qty.toFixed(0)}</div></div>
                        </div>
                    </div>`;
                g.appendChild(card);
            });
