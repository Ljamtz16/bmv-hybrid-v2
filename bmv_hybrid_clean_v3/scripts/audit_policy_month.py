...
def date_span_ok(df, month):
    ...
    end = end.replace(day=1)  # primer día del mes siguiente
...
def main():
    ...
    # Determina el siguiente mes (YYYY-MM)
    dt = datetime.strptime(month + "-01", "%Y-%m-%d")
    if dt.month == 12:
        next_month = f"{dt.year+1}-01"
    else:
        next_month = f"{dt.year:04d}-{dt.month+1:02d}"
