...
# Reusamos la lógica ya corregida del 26_ (BUY/SELL + BOM)
# Si prefieres evitar import circular, puedes copiar apply_policy aquí.
from scripts._reuse_26_policy import apply_policy  # <- ver nota abajo
# NOTA: si no quieres crear scripts/_reuse_26_policy.py, descomenta el bloque
# de "apply_policy_inline" más abajo y elimina la import anterior.

def apply_policy_inline(df: pd.DataFrame, pol: dict) -> pd.DataFrame:
    """Copia local de apply_policy del 26_ (por si prefieres no importar)."""
    out = df.copy()
    def _norm_side(val):
    ...
    tp_pct = float(pol["tp_pct"]); sl_pct = float(pol["sl_pct"])
...
def parse_args():
    ...
    ap.add_argument("--long-only", action="store_true", help="Si se usa, fuerza BUY (opcional)")
...
def main():
    ...
    if args.long_only:
        # fuerza BUY si se pide (opcional)
        df["side"] = "BUY"
    ...
    for tp, sl, H in itertools.product(grid_tp, grid_sl, grid_h):
        pol = dict(
            month=args.month, tp_pct=tp, sl_pct=sl, horizon_days=H,
            min_abs_y=args.min_abs_y, long_only=True,
            per_trade_cash=args.per_trade_cash, commission_side=args.commission_side
        )
        out = apply_policy_inline(df, pol) if args.use_inline else apply_policy(df, pol)
